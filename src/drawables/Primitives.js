...
let createPlaneArray = function (
  lx = -0.5, ly = 0.0, lz = -0.5,
  wx                      = 1.0, wy            = 0.0, wz = 0.0,
  hx                      = 0.0, hy = 0.0, hz = 1.0
) {

  let faces = new Float32Array(4);
  faces[0] = 0;
  faces[1] = 1;
  faces[2] = 2;
  faces[3] = 3;

  let v = new Float32Array(12);
  v[0] = lx;
  v[1] = ly;
  v[2] = lz;
...
let createCubeArray = function (side = 1.0) {
  let v = new Float32Array(24);
  ...
  let f = new Uint32Array(24);
  let ft = new Uint32Array(24);
  f[0] = f[8] = f[21] = ft[0] = 0;
...
let createCylinderArray = function (
  radiusTop                                                    = 0.5, radiusBottom = 0.5, height                  = 2.0,
  radSegments = 64, heightSegments = 64, topCap = true, lowCap = true
) {

  let isSingularTop = topCap && radiusTop === 0.0;
  let isSingularBottom = lowCap && radiusBottom === 0.0;
  let heightHalf = height*0.5;

  let nbVertices = (heightSegments + 1)*radSegments;
  let nbFaces = heightSegments*radSegments;
  if (topCap) {
    nbVertices += 1;
    nbFaces += radSegments;
  }
  ...
  let vAr = new Float32Array(nbVertices*3);
  let fAr = new Uint32Array(nbFaces*4);

  let id = 0;
  let k = 0;
  let i = 0;
  let j = 0;
  let startHeight = isSingularTop ? 1 : 0;
  let endHeight = isSingularBottom ? heightSegments - 1 : heightSegments;
  for (i = startHeight; i <= endHeight; i++) {
    let v = i/heightSegments;
    let radius = v*(radiusBottom - radiusTop) + radiusTop;
    for (j = 0; j < radSegments; j++) {
      let u = Math.PI*2*j/radSegments;
      k = 3*id++;
...
let createTorusArray = function (radiusOut = 0.5, radiusWidth = 0.1, arc = Math.PI*2, nbRadial = 32, nbTubular = 128) {
  let isFull = Math.PI*2 - arc < 1e-2;

  let nbVertices = nbRadial*nbTubular;
  let nbFaces = nbVertices;
  if (!isFull) {
    nbVertices += 2;
    nbFaces += nbRadial;
  }
  let endTubular = isFull ? nbTubular : nbTubular - 1;
  ...
  for (i = 0; i < nbTubular; ++i) {
    for (j = 0; j < nbRadial; ++j) {
      let u = i/endTubular*arc;
      let v = j/nbRadial*Math.PI*2;
      k = 3*id++;
  ...
  for (i = 0; i < endTubular; ++i) {
    let offi = i === nbTubular - 1 ? 0 : i + 1;
    for (j = 0; j < nbRadial; ++j) {
      k = 4*id++;
      fAr[k] = nbRadial*i + j;
      let offj = j === nbRadial - 1 ? 0 : j + 1;
...
let createGridArray = function (
  ...
  let vAr = new Float32Array((res1 + res2)*2*3);
  let i = 0;
  let j = 0;
  let sx = wx/(res1 - 1);
  let sy = wy/(res1 - 1);
...
let createMesh = function (gl, arr) {
...
Primitives.createGrid = function (gl) {
  let mesh = createMesh(gl, createGridArray.apply(this, slice.call(arguments, 1)));
  if (gl) {
...
Primitives.createArrow = function (gl, thick = 0.5, height = 2.0, rConeT = 5.0, rConeH = 0.2, radSegments = 4,
                                   heightSegments = 1) {
  let base = createMesh(null, createCylinderArray(thick, thick, height, radSegments, heightSegments));
  let cone = createMesh(null, createCylinderArray(0.0, thick*rConeT, height*rConeH, radSegments, heightSegments));
