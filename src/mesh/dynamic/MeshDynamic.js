...
class MeshDynamic extends Mesh {
  ...
  updateWireframe(iFaces) {
    let wire = this._wireframe;
    let tris = this.getTriangles();
    let full = iFaces === undefined;
    let useDA = this.isUsingDrawArrays();
    let nbTriangles = full ? this.getNbTriangles() : iFaces.length;
    for (let i = 0; i < nbTriangles; ++i) {
  ...
  updateVerticesOnEdge(iVerts) {
    let vOnEdge = this.getVerticesOnEdge();
    let vrings = this.getVerticesRingVert();
  ...
  resizeArray(orig, targetSize) {
    ...
    // expand
    let tmp = new orig.constructor(targetSize*2);
    tmp.set(orig);
  ...
  computeRingVertices(iVert) {
    ...
    let fAr = this.getFaces();
    let vflags = this.getVerticesTagFlags();

    let vring = this._meshData._vertRingVert[iVert];
    let fring = this._meshData._vertRingFace[iVert];
    vring.length = 0;
    let nbTris = fring.length;
