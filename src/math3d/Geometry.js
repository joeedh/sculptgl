...
Geometry.mouseOnUnitSphere = function (mouseXY) {
  ...
  let tempZ = 1.0 - mouseX*mouseX - mouseY*mouseY;
  let mouseZ = tempZ > 0.0 ? Math.sqrt(tempZ) : 0.0;
...
Geometry.intersectionRayTriangleEdges = (function () {
  ...
  let ONE_PLUS_EPSILON = 1.0 + EPSILON;
  let ZERO_MINUS_EPSILON = 0.0 - EPSILON;
  let pvec = [0.0, 0.0, 0.0];
...
Geometry.intersectionRayTriangle = (function () {
  let edge1 = [0.0, 0.0, 0.0];
  let edge2 = [0.0, 0.0, 0.0];
...
Geometry.distance2PointTriangleEdges = (function () {
  ...
  return function (point, edge1, edge2, v1, a00, a01, a11, closest) {
    ...
    let b0 = vec3.dot(diff, edge1);
    let b1 = vec3.dot(diff, edge2);
    let c = vec3.sqrLen(diff);
...
Geometry.distance2PointTriangle = (function () {
  ...
  return function (point, v1, v2, v3, closest) {
    vec3.sub(edge1, v2, v1);
    vec3.sub(edge2, v3, v1);
    let a00 = vec3.sqrLen(edge1);
    let a01 = vec3.dot(edge1, edge2);
    let a11 = vec3.sqrLen(edge2);
...
Geometry.pointInsideTriangle = (function () {
  ...
  let vecP1 = [0.0, 0.0, 0.0];
  let vecP2 = [0.0, 0.0, 0.0];
...
Geometry.vertexOnLine = (function () {
  ...
  return function (vertex, vNear, vFar) {
    ...
    let dot = vec3.dot(ab, vec3.sub(proj, vertex, vNear));
...
Geometry.intersectLinePlane = function (s1, s2, origin, normal, out) {
  let dist1 = vec3.dot(vec3.sub(out, s1, origin), normal);
  let dist2 = vec3.dot(vec3.sub(out, s2, origin), normal);
